...
- 装饰器，装饰器本质上是一个**高阶函数**，可作用于类、方法、访问符（getter\setter）、类属性以及方法参数，可以获取并修改被作用元素的元信息。
	...
	- 类装饰器（Class Decorator）
	  类装饰器会在运行时被当做函数被调用，**类的构造函数**作为其唯一参数
	  ```typescript
	  type ClassConstructor = new (...args: any[]) => any
	  type ClassDecorator = <T extends ClassContructor>(target: T) => T | void
	  ...
	  ```
	- 属性装饰器（Prototype Decorator）
	  属性装饰器会在运行时当做函数被调用，参数包括：
	  1. `target` 对于
	- 方法装饰器（Method Decorator）
	- 参数装饰器（Parammeter Decorator）
- 元数据
  collapsed:: true
	- 元数据是“关于数据的数据”，在Typescript中，元数据是附加在**类、方法、属性、参数等上的额外信息**，信息不会影响代码执行逻辑，但可以在**运行时**被读取和使用。
	- `reflect-metadata`库是用于支持`Reflect`一些实验性metadata API。
	- 在`Tyepscript`中，开启`emitDecoratorMetadata(tsconfig.json)`配置项，ts会自动为*带有装饰器的声明*生成元数据。
